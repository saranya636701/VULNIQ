...
```powershell
python 4_cluster_enrichment.py
```

### Final Results and Visualization

```powershell
python 5_final_visualization.py
```

The scripts should be executed sequentially because later stages use datasets generated by earlier stages.
...
Instead of treating CVSS severity as the sole basis for understanding vulnerabilities, the project combines severity, exploitability, impact, exploitation probability, known exploitation status, attack characteristics and vulnerability age.

Unsupervised machine learning is then used to identify groups of vulnerabilities with similar characteristics.

The final K-Means model identifies four vulnerability profiles, while threat intelligence enrichment using CWE, vendor and product information makes these clusters easier to interpret from a cybersecurity perspective.

The most significant finding is the identification of a small cluster containing a disproportionately large percentage of CISA Known Exploited Vulnerabilities, demonstrating the potential value of multidimensional vulnerability profiling beyond severity-based classification.
